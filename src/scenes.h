...
hittable_list simple_light() {
	...
	auto perlin_text = std::make_shared<noise_texture>(4);
	objects.add(std::make_shared<sphere>(point3(0, -1000, 0), 1000, std::make_shared<lambertian>(perlin_text)));
	objects.add(std::make_shared<sphere>(point3(0, 2, 0), 2, std::make_shared<lambertian>(perlin_text)));

	auto diff_light = std::make_shared<diffuse_light>(color(4, 4, 4));
	auto dim_light = std::make_shared<diffuse_light>(color(2, 2, 2));

	objects.add(std::make_shared<xy_rect>(3, 5, 1, 3, -2, diff_light));
	objects.add(std::make_shared<sphere>(point3(5, 6, 4), 1, dim_light));
...
hittable_list two_perlin_spheres() {
	...
	objects.add(std::make_shared<sphere>(point3(0, -10, 0), 10, std::make_shared<lambertian>(perlin_texture)));
	objects.add(std::make_shared<sphere>(point3(0, 10, 0), 10, std::make_shared<lambertian>(perlin_texture)));
...
hittable_list feature_ensemble() {
	...
	for (int i = 0; i < boxes_per_side; i++) {
		for (int j = 0; j < boxes_per_side; j++) {
			...
			floor_boxes.add(std::make_shared<box>(point3(x0, y0, z0), point3(x1, y1, z1), ground));
		}
	}

	hittable_list objects;

	objects.add(std::make_shared<bvh_node>(floor_boxes, 0, 1));
	...
	auto earth = std::make_shared<lambertian>(std::make_shared<image_texture>("earthmap.jpg"));
	objects.add(std::make_shared<sphere>(point3(400, 200, 400), 100, earth));

	auto perlin_texture = std::make_shared<noise_texture>(0.1);
	...
	int n = 1000;
	for (int j = 0; j < n; j++) {
		sphere_cube.add(std::make_shared<sphere>(point3::random(0, 165), 10, white));
	}

	objects.add(std::make_shared<translate>(
		std::make_shared<rotate_y>(
			std::make_shared<bvh_node>(sphere_cube, 0.0, 1.0), 15),
		vec3(-100, 270, 395)
		)
	);
