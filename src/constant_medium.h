...
bool constant_medium::hit(const ray& r, double t_min, double t_max, hit_record& rec) const {
	...
	const bool debugging = enable_debugging && random_double() < 0.00001;
	...
	const auto length_inside_boundary = (rec2.t - rec1.t) * ray_length;
	const auto hit_distance = neg_inv_density * log(random_double());

	if (hit_distance > length_inside_boundary)
		return false;
